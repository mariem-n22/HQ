 ...
 *
 * Visually distinct from the photography grid on purpose: drawings are
 * documents, so they sit on a flat surface, are never cropped (`contain`), and
 * are labelled. The viewer opens in zoom mode because a drawing you cannot
 * enlarge is decoration.
 */
export function DrawingSet({ groups }: { groups: { label: string; items: MediaItem[] }[] }) {
  const present = groups.filter((g) => g.items.length > 0);
  const flat = present.flatMap((g) => g.items.map((item) => ({ ...item, label: item.label || g.label })));
  const [open, setOpen] = useState<number | null>(null);
  ...
  return (
    <section aria-label="Drawings">
                ...
                return (
                  <li key={`${item.url}-${i}`}>
                    <button
                      ...
                      onClick={() => setOpen(at)}
                      className="group block w-full text-left focus-visible:outline-2 focus-visible:outline-offset-4 focus-visible:outline-amber"
                      aria-label={`Enlarge ${item.label || group.label} ${i + 1}`}
                    >
                      <MediaFigure
                        item={item}
                        fallbackRatio="4 / 3"
                    ...
                    {item.caption || item.label ? (
                      <p className="mt-2 text-xs text-mute">{item.caption ?? item.label}</p>
                    ) : null}
                  </li>
                );
              })}
      ...
      <MediaViewer items={flat} index={open} onClose={() => setOpen(null)} onIndex={setOpen} zoomable />
    </section>
  );
}

/** Concept diagrams — a small ordered sequence, numbered so the order reads. */
      ...
      <ol className="mt-6 grid gap-6 sm:grid-cols-2 lg:grid-cols-4">
        {items.map((item, i) => (
          <li key={`${item.url}-${i}`}>
            ...
            <p className="mt-2 flex gap-2 text-xs text-mute">
